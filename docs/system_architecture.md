...
- Flue Gas Analyzers (Excess Oxygen % O2, Flue Gas Stack Temp).
...
- Real-time heat transfer rate calculation: `Q_water = m_dot_w * cp * (T_out - T_in)`.
...
- PyTorch PINN inverse model: estimates unobservable latent fouling resistance Rf(t) and heat transfer coefficient U(t).
- Continuous physics residual evaluation: `dQ = |Q_in - Q_out - Q_loss|`.
...
- What-If Scenario Sandbox: enables operators to simulate load derating (e.g. 90% to 75%) to extend asset survival.
